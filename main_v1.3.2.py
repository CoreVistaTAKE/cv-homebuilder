...
.pv-shell.pv-layout-260218{
  --pv-depth-overscan-x: max(22vw, 320px);
  --pv-depth-overscan-y: max(18vh, 180px);
  position: relative;
  isolation: isolate;
  width: 100%;
  max-width: 100%;
...
body.pv-page-body > .pv-shell.pv-layout-260218:not(.pv-preview-live){
  --pv-depth-overscan-x: max(46vw, 720px);
  --pv-depth-overscan-y: max(26vh, 280px);
}
...
def render_preview(p: dict, mode: str = "pc", *, root_id: Optional[str] = None, in_builder: bool = False) -> None:
    ...
    def _builder_preview_dims(kind: str) -> tuple[int, int]:
        # ON は「約10%前後の軽量表示」、OFF は「約75%前後の詳細表示」
        if kind == "favicon":
            return (24, 24) if preview_light_images else (64, 64)
        if kind == "logo":
            return (96, 32) if preview_light_images else (300, 96)
        if kind == "hero":
            if mode == "pc":
                return (240, 135) if preview_light_images else (768, 432)
            return (180, 101) if preview_light_images else (540, 304)
        if kind in {"about", "services"}:
            if mode == "pc":
                return (264, 149) if preview_light_images else (840, 473)
            return (180, 101) if preview_light_images else (540, 304)
        return (240, 135) if preview_light_images else (768, 432)
    ...
    builder_class = (" pv-preview-live pv-preview-lite" if (in_builder and preview_light_images) else (" pv-preview-live" if in_builder else ""))
        ...
        with ui.element("div").classes("pv-scroll pv-scroll-live" if in_builder else "pv-scroll"):
                        ...
                        with ui.element("div").classes("pv-hero-track"):
                            for url in hero_urls:
                                with ui.element("div").classes("pv-hero-slide"):
                                    _hero_dims = (_builder_preview_dims("hero") if in_builder else ((1024, 576) if mode == "pc" else (720, 405)))
            ...
            with ui.element("main").classes("pv-main"):
                ...
                with ui.element("section").classes("pv-section pv-section-260218").props('id="pv-about"'):
                    ...
                    with ui.element("div").classes("pv-panel pv-panel-glass"):
                        if about_image_url:
                            _about_dims = (_builder_preview_dims("about") if in_builder else ((1100, 619) if mode == "pc" else (720, 405)))
                ...
                with ui.element("section").classes("pv-section pv-section-260218").props('id="pv-services"'):
                    ...
                    with ui.element("div").classes("pv-panel pv-panel-glass"):
                        if svc_image_url:
                            _svc_dims = (_builder_preview_dims("services") if in_builder else ((1100, 619) if mode == "pc" else (720, 405)))
...
def render_main(u: User) -> None:
    ...
    def _preview_debounce_sec() -> float:
        try:
            ...
            return 0.24 if light_on else 0.40
        except Exception:
            return 0.32
                                ...
                                with ui.tab_panels(step_tabs, value=step_initial).classes("w-full"):
                                    ...
                                    with ui.tab_panel("s1"):
                                        ...
                                        with ui.card().classes("q-pa-sm rounded-borders w-full q-mb-sm cvhb-setting-card").props("flat bordered"):
                                            ...
                                            ui.label("編集中だけ画像を軽く表示できます。ON は約10%前後の軽量表示、OFF は約75%前後の詳細表示です。完成したHPの画質には影響しません。").classes("cvhb-setting-note q-mb-xs")
