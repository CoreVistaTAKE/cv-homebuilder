...
def render_preview(p: dict, mode: str = "mobile") -> None:
    ...
    def section_title(icon_name: str, title: str) -> None:
        with ui.row().classes("items-center q-gutter-sm q-mb-sm"):
            # 白/黄のときは text-primary が見えにくいので少し濃い色に
    ...
    if mode == "mobile":
        with ui.column().classes("w-full"):
            with ui.element("div").classes(f"w-full bg-{primary} {_safe_primary_text_class(primary)}"):
                with ui.row().classes("items-center justify-between q-px-md").style("height: 48px;"):
                    ui.label(company).classes("text-subtitle2")
                    ui.icon("menu").classes(_safe_primary_text_class(primary))
                ...
                with ui.column().classes("q-pa-md"):
                    ...
                    if pts:
                        with ui.row().classes("q-gutter-xs q-mt-sm"):
                            for t in pts:
                                ui.badge(t).props("outline").classes(f"text-{primary}" if primary not in {"white"} else "text-grey-9")
                ...
                with ui.element("div").classes("cvhb-left-col"):
                    with ui.element("div").classes("cvhb-left-stack"):
                        ...
                        with ui.card().classes("q-pa-md rounded-borders").props("bordered"):
                            ...
                            with ui.tabs().props("vertical dense").classes("w-full") as step_tabs:
                            ...
                            if not p:
                                ...
                                def bind_step2_input(label: str, key: str, hint: str = "") -> None:
                                    ...
                                    def _on_change(e):
                                        ...
                                        update_and_refresh()
                                    inp = ui.input(label, value=val, on_change=_on_change).props("outlined").classes("w-full q-mb-sm")
                                    if hint:
                                            ...
                                            def industry_selector():
                                                ...
                                                for opt in INDUSTRY_PRESETS:
                                                    ...
                                                    card = ui.card().classes(
                                                        "q-pa-md q-mb-sm cvhb-choice " + ("is-selected" if selected else "")
                                                ...
                                                for opt in COLOR_PRESETS:
                                                    ...
                                                    with card:
                                                        with ui.row().classes("items-center justify-between"):
                                                            ...
                                                            if selected:
                                                                ui.icon("check_circle").classes("text-primary")
                                                        ui.label(f"※ {opt['label']}が与える印象：{opt['impression']}").classes("cvhb-muted q-mt-xs")
...
def projects_page():
    ...
    with ui.element("div").classes("cvhb-container"):
        ui.label("案件一覧").classes("text-h5 q-mb-md")
        ...
        with ui.row().classes("q-gutter-sm q-mb-md").style("flex-wrap: wrap;"):
        ...
        def list_refresh():
            ...
            if not items:
                ui.label("案件がありません").classes("cvhb-muted")
                return
            ...
            for it in items:
                ...
                with ui.card().classes("q-pa-md q-mb-sm rounded-borders").props("bordered"):
                    with ui.row().classes("items-center justify-between"):
                        with ui.column():
                            ui.label(pname).classes("text-subtitle1")
                            ui.label(f"ID: {pid}").classes("cvhb-muted")
                            ui.label(f"案件開始日: {created_at}").classes("cvhb-muted")
                            ui.label(f"最新更新日: {updated_at}").classes("cvhb-muted")
                            if updated_by:
                                ui.label(f"更新担当者: {updated_by}").classes("cvhb-muted")
                        ui.button("開く", on_click=lambda project_id=pid: open_project(project_id)).props("color=primary unelevated")
