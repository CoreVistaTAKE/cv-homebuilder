    ...
    with ui.element("div").classes("w-full cvhb-appbar"):
        with ui.row().classes("w-full items-center justify-between cvhb-appbar-inner"):
            with ui.row().classes("items-center no-wrap cvhb-brand-lockup"):
                ui.icon("dashboard_customize").classes("text-primary")
                with ui.column().classes("no-wrap"):
                    ui.label(f"{BUILDER_NAME} v{VERSION}").classes("cvhb-brand-title")
                    ui.label(f"{PRODUCT_NAME}｜{BUILDER_NAME_JA}").classes("cvhb-brand-sub")
...
def render_login(root_refresh) -> None:
    with ui.element("div").classes("cvhb-auth-shell"):
        with ui.card().classes("q-pa-xl rounded-borders cvhb-auth-card").props("bordered"):
            ...
            ui.button("ログイン", on_click=do_login).props("color=primary unelevated no-caps").classes("q-mt-md w-full")


def render_first_admin_setup(root_refresh) -> None:
    with ui.element("div").classes("cvhb-auth-shell"):
        with ui.card().classes("q-pa-xl rounded-borders cvhb-auth-card").style("width: 620px; max-width: 92vw;").props("bordered"):
            ui.label(PRODUCT_NAME).classes("cvhb-panel-caption")
            ...
            ui.button("管理者を作成", on_click=create_admin).props("color=primary unelevated no-caps").classes("q-mt-md w-full")
    ...
    def render_startup_loading(title: str, detail: str = "少しお待ちください。") -> None:
        ...
        with root:
            with ui.element("div").classes("w-full").style(
                "min-height: calc(100vh - 0px);"
                "background: radial-gradient(640px 280px at 10% 10%, rgba(59,130,246,0.08), transparent 72%),"
                "radial-gradient(520px 240px at 92% 12%, rgba(139,92,246,0.07), transparent 72%),"
                "linear-gradient(180deg, rgba(245,247,251,1), rgba(238,244,255,1));"
            ):
                with ui.column().classes("w-full items-center justify-center text-center q-pa-xl").style("min-height: 68vh;"):
                    with ui.card().classes("q-pa-xl rounded-borders cvhb-loading-card cvhb-surface-card").style("width: 440px; max-width: 92vw;").props("bordered"):
                        with ui.column().classes("w-full items-center justify-center text-center"):
