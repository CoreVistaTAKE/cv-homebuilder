...
def render_preview(p: dict, mode: str = "pc", *, root_id: Optional[str] = None, in_builder: bool = False) -> None:
    ...
    def _builder_preview_dims(kind: str) -> tuple[int, int]:
        ...
        """
        if kind == "favicon":
            return (16, 16) if preview_light_images else (48, 48)
        if kind == "logo":
            return (60, 20) if preview_light_images else (256, 82)
        if kind in {"hero", "about", "services"}:
            return (320, 180) if preview_light_images else (768, 432)
        return (240, 135) if preview_light_images else (640, 360)
            ...
            with ui.element("div").classes("cvhb-container"):
                    ...
                    def images_dialog_body():
                        ...
                        with ui.element("div").classes("q-mt-sm"):
                            for it in items:
                                ...
                                size_kb = int(it.get("size_kb") or 0)
                                preview_url = str(it.get("thumb_data_url") or it.get("data_url") or "")

                                with ui.row().classes("items-center q-gutter-sm q-mb-sm"):
                                    try:
                                        if preview_url:
                                            ui.image(pv_img_src(preview_url, max_w=184, max_h=104, fit_mode="cover")).style(
                ...
                with ui.dialog() as img_delete_dialog, ui.card().classes("q-pa-md rounded-borders").props("bordered"):
                    ...
                    def img_delete_body():
                        ...
                        preview_url = str(img_del_state.get("thumb_data_url") or data_url or "")
                        ...
                        with ui.row().classes("items-center q-gutter-sm"):
                            try:
                                if preview_url:
                                    ui.image(pv_img_src(preview_url, max_w=440, max_h=248, fit_mode="cover")).style(
