...
def projects_page():
    ...
    def switch_to_builder_inline() -> bool:
        prev_mode = str(page_state.get("mode") or "list")
        page_state["mode"] = "builder"
        try:
            page_refresh.refresh()
            return True
        except Exception:
            page_state["mode"] = prev_mode
            return False
                    ...
                    async def _create_new_project() -> None:
                        ...
                        try:
                            ...
                            try:
                                ...
                                pass
                            if not switch_to_builder_inline():
                                navigate_to("/")
                        except Exception as e:
                    ...
                    try:
                        ...
                        if project_cache_hit(u, pid):
                            ...
                            if not switch_to_builder_inline():
                                navigate_to("/")
                            return
                        project_obj = await asyncio.to_thread(load_project_from_sftp, pid, u)
                        set_current_project(project_obj, u)
                        clear_pending_open_project()
                        try:
                            open_project_dialog.close()
...
"""
    ...
    if needs_builder_loading:
        ...
        if pending_project_id:
            ...
            detail = f"案件を読み込んでから {BUILDER_NAME} を表示します。"
        ...
        with sftp_client() as sftp:
            ...
            for d in dirs:
                ...
                if not isinstance(meta, dict) or not meta:
                    # fallback to the lighter original listing for legacy data
                    try:
                        project_obj = _load_project_from_sftp_v173(d, None)
                        project_obj = normalize_project(project_obj)
                        meta = _build_project_meta(project_obj)
                    except Exception:
                        meta = {"project_id": d, "project_name": "(broken project.json)", "updated_at": "", "created_at": "", "updated_by": ""}
