...
def create_project(name: str, created_by: Optional[User]) -> dict:
    ...
    p = {
        ...
        "data": {
            "step1": {"industry": "会社サイト（企業）", "primary_color": "blue", "welfare_domain": "", "welfare_mode": "", "template_id": "corp_v1", "preview_light_images": True},
...
"""
    ...
    for _row in _company_profile_visible_extra_rows(profile):
        ...
        if not _val:
            continue
        _val_html = html.escape(_val).replace("\n", "<br>")
...
def _header_brand_html() -> str:
    ...
    sub = html.escape(f"{BUILDER_NAME} v{VERSION}｜{BUILDER_NAME_JA}")
    return f"""
...
def render_preview(p: dict, mode: str = "pc", *, root_id: Optional[str] = None, in_builder: bool = False) -> None:
    ...
    for item in svc_items:
        ...
        if not t and not b:
            continue
        body_html = html.escape(b).replace("\n", "<br>") if b else ""
        svc_preview_cards.append(
            f'<div class="pv-service-item">'
            f'{f"<div class=\"pv-service-title\">{html.escape(t)}</div>" if t else ""}'
            f'{f"<div class=\"pv-service-body\">{body_html}</div>" if body_html else ""}'
            f'</div>'
        )
    ...
    services_panel_preview_html = build_services_panel_markup(
        title=svc_title,
        lead_html=html.escape(svc_lead).replace("\n", "<br>") if svc_lead else "",
    ...
    recruitment_rows_preview_html = "".join(
        [
            f'<div class="pv-company-profile-row"><div class="pv-company-profile-label">{html.escape(lbl)}</div><div class="pv-company-profile-value">{html.escape(val).replace("\n", "<br>")}</div></div>'
            for lbl, val in recruitment_rows
        ]
    )
    recruitment_panel_preview_html = build_recruitment_panel_markup(
        lead_html=html.escape(recruitment_lead).replace("\n", "<br>") if recruitment_lead else "",
                                ...
                                def update_and_refresh() -> None:
                                    ...
                                    if after_tpl != before_tpl:
                                        try:
                                            ...
                                            pass
                                    refresh_preview()
                                    refresh_approval_panel()
                                    refresh_publish_panel()
    ...
    def page_refresh():
        ...
        with page_root:
            ...
            with ui.element("div").classes("cvhb-container"):
                ...
                with ui.dialog() as pack_create_dialog, ui.card().classes("q-pa-md rounded-borders").props("bordered"):
                    ...
                    def _open_pack_create_dialog() -> None:
                        ...
                        if current_name:
                            pack_current_hint.text = f"現在の案件: {current_name}"
                        else:
                            pack_current_hint.text = "現在開いている案件がない場合は、空のテンプレパックを作ります。"
                ...
                with ui.dialog() as pack_import_dialog, ui.card().classes("q-pa-md rounded-borders").props("bordered"):
                    ...
                    async def _on_pack_zip_upload(e) -> None:
                        ...
                        try:
                            ...
                            p_pack, pack_summary = await asyncio.to_thread(build_project_from_pageflow_pack_zip, raw, u)
                            await asyncio.to_thread(save_project_to_sftp, p_pack, u)
                            set_current_project(p_pack, u)

                            try:
                                safe_log_action(
                                    u,
                                    "project_pack_import_create",
                                    details=json.dumps(pack_summary, ensure_ascii=False),
                                )
                            except Exception:
                                pass
                ...
                with ui.card().classes("q-pa-md rounded-borders q-mb-md cvhb-toolbar-card").props("bordered"):
                    with ui.row().classes("items-center justify-between").style("gap: 12px;"):
                        with ui.column():
                            ...
                            ui.label(f"案件を開いて {BUILDER_NAME} で編集します。新規作成もここからできます。").classes("cvhb-muted")
                ...
                # =========================
                img_state = {"loading": False, "error": "", "items": [], "project_id": "", "project_name": "", "source": ""}
                del_state = {"project_id": "", "project_name": ""}
                img_del_state = {"label": "", "filename": "", "mime": "", "size_kb": 0, "data_url": "", "data_sha1": "", "thumb_data_url": ""}
                    ...
                    def images_dialog_body():
                        ...
                        def _open_image_delete(item: dict) -> None:
                            ...
                            try:
                                ...
                                img_del_state["data_sha1"] = str(item.get("data_sha1") or "")
                                img_del_state["thumb_data_url"] = str(item.get("thumb_data_url") or item.get("data_url") or "")
                            except Exception:
                                img_del_state["label"] = ""
                                ...
                                img_del_state["data_sha1"] = ""
                                img_del_state["thumb_data_url"] = ""
                            # 最終確認チェックは毎回リセット
                            try:
                        ...
                        source = str(img_state.get("source") or "")
                        if source == "meta":
                            ui.label("高速表示中です。画像一覧は軽いメタ情報から開いています。削除時だけ案件本体を読みます。").classes("cvhb-muted text-caption q-mt-xs")
                        ...
                        with ui.element("div").classes("q-mt-sm"):
                            for it in items:
                                ...
                                size_kb = int(it.get("size_kb") or 0)
                                thumb_data_url = str(it.get("thumb_data_url") or "")
                                preview_url = thumb_data_url or str(it.get("data_url") or "")
                                preview_src = pv_img_src(thumb_data_url) if thumb_data_url else pv_img_src(preview_url, max_w=184, max_h=104, fit_mode="cover")

                                with ui.row().classes("items-center q-gutter-sm q-mb-sm"):
                                    try:
                                        if preview_url:
                                            ui.image(preview_src).style(
                ...
                with ui.dialog() as img_delete_dialog, ui.card().classes("q-pa-md rounded-borders").props("bordered"):
                    ...
                    def img_delete_body():
                        ...
                        data_sha1 = str(img_del_state.get("data_sha1") or "")
                        thumb_data_url = str(img_del_state.get("thumb_data_url") or "")
                        preview_url = thumb_data_url or data_url or ""
                        preview_src = pv_img_src(data_url, max_w=440, max_h=248, fit_mode="cover") if data_url else (pv_img_src(thumb_data_url) if thumb_data_url else "")

                        if not preview_url and not data_sha1:
                        ...
                        with ui.row().classes("items-center q-gutter-sm"):
                            ...
                            with ui.column():
                                ...
                                ui.label(f"{fn or '(filenameなし)'} / {mime} / {size_kb}KB").classes("cvhb-muted")
                                if data_sha1:
                                    ui.label(f"識別ID: {data_sha1[:12]}").classes("cvhb-muted text-caption")
                    ...
                    async def _confirm_img_delete():
                        ...
                        target_sha1 = str(img_del_state.get("data_sha1") or "").strip().lower()
                        if not pid or (not target and not target_sha1):
                        ...
                        try:
                            ...
                            def _work():
                                ...
                                if target:
                                    ...
                                    cleared = remove_data_url_from_project_by_sha1(proj, target_sha1)
        ...
        # --- 削除準備ダイアログ（チェック必須） ---
                ...
                async def open_images_dialog(project_id: str, project_name: str) -> None:
                    ...
                    try:
                        ...
                        items = []
                        meta_loaded = isinstance(meta, dict) and ("items" in meta or "image_count" in meta)
                        if meta_loaded:
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    ui.run(
        title=f"{BUILDER_NAME} | {PRODUCT_NAME} v{VERSION}",
