  ...
  ready(function(){
    ...
    if hero_urls_html:
        ...
        for i, u in enumerate(hero_urls_html):
            img_attrs = "loading=\"eager\" fetchpriority=\"high\" decoding=\"async\" width=\"1080\" height=\"608\"" if i == 0 else "loading=\"lazy\" fetchpriority=\"low\" decoding=\"async\" width=\"1080\" height=\"608\""
...
"""
    ...
    about_img_html = f'<div class="pv-media-frame pv-media-frame-about"><img class="pv-about-img" src="{_esc(ph_img_href)}" alt="" loading="lazy" fetchpriority="low" decoding="async" width="1080" height="608"></div>' if ph_img_href else '<div class="pv-media-frame pv-media-frame-about"><div class="pv-about-img pv-about-img-placeholder"></div></div>'
...
"""
    ...
    svc_img_html = f'<div class="pv-media-frame pv-media-frame-services"><img class="pv-services-img" src="{_esc(svc_img_href)}" alt="" loading="lazy" fetchpriority="low" decoding="async" width="1080" height="608"></div>' if svc_img_href else ''
...
def render_preview(p: dict, mode: str = "pc", *, root_id: Optional[str] = None, in_builder: bool = False) -> None:
    ...
    def _builder_preview_dims(kind: str) -> tuple[int, int]:
        ...
        - ON : 軽快さ優先（約10%前後）
        - OFF: 見た目優先（約75%前後）
        - 横幅は モバイル=860px / PC=1080px を基準に、アスペクト比は維持する
        """
        base_w = 1080 if mode == "pc" else 860
        lite_ratio = 0.10
        full_ratio = 0.75
        ...
        ratio = lite_ratio if preview_light_images else full_ratio
        width = max(64, int(round(base_w * ratio)))
        height = max(36, int(round(width * 9 / 16)))
...
def render_main(u: User) -> None:
    ...
    def _preview_debounce_sec() -> float:
        try:
            ...
            return 0.32 if light_on else 0.50
        except Exception:
            return 0.40
