...
def _normalize_preview_light_images(value) -> bool:
    try:
        ...
        if s in {"0", "false", "off", "no", "n", "heavy", "full", "75", "75%"}:
            return False
        if s in {"1", "true", "on", "yes", "y", "light", "25", "25%", ""}:
            return True
    except Exception:
        return True
    return True
...
/* ===== Depth Background Rebuild (v1.2.9) ===== */
.pv-shell.pv-layout-260218{
  --pv-depth-overscan-x: max(18vw, 240px);
...
def render_preview(p: dict, mode: str = "pc", *, root_id: Optional[str] = None, in_builder: bool = False) -> None:
    ...
    def _builder_preview_dims(kind: str) -> tuple[int, int]:
        if kind == "favicon":
            return (40, 40) if preview_light_images else (64, 64)
        if kind == "logo":
            return (180, 60) if preview_light_images else (300, 96)
        if kind == "hero":
            if mode == "pc":
                return (320, 180) if preview_light_images else (768, 432)
            return (240, 135) if preview_light_images else (540, 304)
        if kind in {"about", "services"}:
            if mode == "pc":
                return (352, 198) if preview_light_images else (840, 473)
            return (240, 135) if preview_light_images else (540, 304)
        return (320, 180) if preview_light_images else (768, 432)
                                ...
                                with ui.tab_panels(step_tabs, value=step_initial).classes("w-full"):
                                    ...
                                    with ui.tab_panel("s1"):
                                        ...
                                        with ui.card().classes("q-pa-sm rounded-borders w-full q-mb-sm cvhb-setting-card").props("flat bordered"):
                                            ...
                                            ui.label("編集中だけ画像を軽く表示できます。ON は約25%の軽量表示、OFF は約75%の詳細表示です。完成したHPの画質には影響しません。").classes("cvhb-setting-note q-mb-xs")
