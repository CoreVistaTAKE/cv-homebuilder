...
"""
    ...
    if favicon_href_html and ("?" not in favicon_href_html) and favicon_href_html.startswith("assets/"):
        favicon_href_html = f"{favicon_href_html}?v={CURRENT_APP_VERSION}"
                            ...
                            with ui.card().classes(f"cvhb-preview-card cvhb-preview-card-{mode} cvhb-surface-card").style(frame_style).props("flat bordered"):
                                with ui.element("div").classes(f"cvhb-preview-stage cvhb-preview-stage-{mode}").props(fit_props).style(fit_style):
                                    ...
                                    try:
                                        ...
                                        try:
                                            ui.run_javascript(
                                                "setTimeout(function(){ window.cvhbInitPreviewScrollBridge && window.cvhbInitPreviewScrollBridge('pv-root'); }, 0);"
                                            )
                                        except Exception:
                                            pass
